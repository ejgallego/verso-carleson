...
$$`B(\pc(\fp), 8D^{\ps(\fp)})`
with $`\fp\in \mathfrak{A}` and recall the maximal-function definition and
estimate for $`M_{\mathcal{B}}`.
...
Then the Dirichlet approximation estimate shows that the kernels of $`H_r` and $`2\pi L_N`
